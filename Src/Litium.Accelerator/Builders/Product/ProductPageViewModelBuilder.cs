...
namespace Litium.Accelerator.Builders.Product
{
    public class ProductPageViewModelBuilder : IViewModelBuilder<ProductPageViewModel>
    {
        ...
        public ProductPageViewModel Build(CategoryModel categoryModel, BaseProduct baseProduct)
        {
            ...
            viewModel.MostSoldProducts = GetMostSoldProducts(categoryModel.SystemId, baseProduct.Id);

            return viewModel;
        }

        public ProductPageViewModel Build(CategoryModel categoryModel, Variant variant)
        {
            var productModel = _productModelBuilder.BuildFromVariant(variant);
            var viewModel = Build(productModel);

            viewModel.BundleProducts = GetBundles(variant);
            viewModel.MostSoldProducts = GetMostSoldProducts(categoryModel.SystemId, variant.Id);
        ...
        private List<ProductItemViewModel> GetMostSoldProducts(Guid categorySystemId, string currentVariantId)
        {
            return _productService
                .GetMostSoldProducts(_requestModelAccessor.RequestModel.WebsiteModel.SystemId, _requestModelAccessor.RequestModel.ChannelModel.SystemId, 
                                    productGroupIds: new[] { categorySystemId }, ignoreVariantId: currentVariantId)
