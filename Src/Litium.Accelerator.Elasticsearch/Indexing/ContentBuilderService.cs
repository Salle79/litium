...
namespace Litium.Accelerator.Indecies
{
    ...
    public class ContentBuilderService
    {
            ...
            where TTemplate : FieldTemplate where TArea : IArea
        {
            ...
            var fields = _fieldCache.GetOrAdd((typeof(TArea), templateId),
                key => _templateSettingService.GetTemplateIndexingFields<TArea>(templateId)?.Select(x => _fieldDefinitionService.Get<TArea>(x))
                                              .Where(x => x is object)
                                              .ToList())
                ?? _fieldCache.GetOrAdd((typeof(TArea), null), _ => _fieldDefinitionService.GetAll<TArea>().ToList());
...
#pragma warning restore CS0618 // Type or member is obsolete
                    ...
                    case SystemFieldTypeConstants.DecimalOption:
                        ...
                        {
                            foreach (var item in GetDecimalOptionValue((DecimalOption)fieldDefinition.Option, decimalValues))
                            {
                                content.Add(item);
                            }
                        }
                    ...
                    case SystemFieldTypeConstants.IntOption:
                        ...
                        {
                            foreach (var item in GetIntOptionValue((IntOption)fieldDefinition.Option, intValues))
                            {
                                content.Add(item);
                            }
                        }
                    ...
                    case SystemFieldTypeConstants.TextOption:
                        ...
                        {
                            foreach (var item in GetTextOptionValue((TextOption)fieldDefinition.Option, textValues))
                            {
                                content.Add(item);
                            }
                        }
                        ...
                        {
                            goto case SystemFieldTypeConstants.Text;
                        }
                        break;
            ...
            IEnumerable<string> GetDecimalOptionValue(DecimalOption options, IList<decimal> values)
            {
            ...
            IEnumerable<string> GetIntOptionValue(IntOption options, IList<int> values)
            {
                return values.Select(value =>
                {
                    var option = options?.Items?.FirstOrDefault(x => x.Value == value);
            ...
            IEnumerable<string> GetTextOptionValue(TextOption options, IList<string> values)
            {
                return values.Select(value =>
                {
                    var option = options?.Items?.FirstOrDefault(x => x.Value == value);
