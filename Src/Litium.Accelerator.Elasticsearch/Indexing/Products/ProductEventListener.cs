...
namespace Litium.Accelerator.Search.Indexing.Products
{
    public class ProductEventListener : IIndexQueueHandlerRegistration
    {
        ...
        public ProductEventListener(EventBroker eventBroker, IndexQueueService indexQueueService, VariantService variantService, CategoryService categoryService)
        {
            ...
            _eventBroker.Subscribe<VariantToChannelLinkAdded>(x => UpdateVariant(x.VariantSystemId));
            _eventBroker.Subscribe<VariantToChannelLinkRemoved>(x => UpdateVariant(x.VariantSystemId));
        }
