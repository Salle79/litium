...
#pragma warning restore CS0618 // Type or member is obsolete
                    ...
                    case SystemFieldTypeConstants.Decimal:
                        foreach (var value in values)
                        {
                            model.Tags.Add(new ProductDocument.TagItem { Key = filterField, Value = ((decimal)value).ToString(cultureInfo) });
                        }
                        break;
                    ...
                    case SystemFieldTypeConstants.DecimalOption:
                        foreach (var value in values)
                        {
                            ...
                            {
                                foreach (var item in GetDecimalOptionValue((DecimalOption)field.Option, decimalValues))
                                {
                    ...
                    case SystemFieldTypeConstants.IntOption:
                        foreach (var value in values)
                        {
                            ...
                            {
                                foreach (var item in GetIntOptionValue((IntOption)field.Option, intValues))
                                {
                                    AddOptionTag(model, filterField, item);
                                }
                            }
                    ...
                    case SystemFieldTypeConstants.TextOption:
                        foreach (var value in values)
                        {
                            ...
                            {
                                foreach (var item in GetTextOptionValue((TextOption)field.Option, textValues))
                                {
                                    AddOptionTag(model, filterField, item);
                                }
                            }
